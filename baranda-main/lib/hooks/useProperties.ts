...
type ProfileRowFull = ProfileRowPublic & { phone_e164: string | null; bio: string | null };
...
type PropertyRowDetail = PropertyRowBase & { profiles: ProfileRowFull | null };

function rowToProperty(row: PropertyRow | PropertyRowDetail): Property {
  const profile = row.profiles as Partial<ProfileRowFull> | null;
  return {
    id: row.id,
    purpose: row.purpose,
    type: row.type,
...
const SELECT_DETAIL = "*, profiles!properties_seller_profile_fkey(*)";
...
async function fetchPropertyDetail(id: string): Promise<Property | null> {
  ...
  if (error) throw error;
  if (!data) return null;
  return rowToProperty(data as unknown as PropertyRowDetail);
}
...
export function useUpdateProperty() {
  ...
  return useMutation({
    mutationFn: async ({ id, patch }: { id: string; patch: Partial<CreatePropertyInput> }) => {
      ...
      const { error } = await supabase.from("properties").update(row as any).eq("id", id);
      if (error) throw error;
    },
...
export function useTogglePinProperty() {
  ...
  return useMutation({
    mutationFn: async ({ id, pinned }: { id: string; pinned: boolean }) => {
      const { error } = await supabase.from("properties").update({ pinned, pinned_at: pinned ? new Date().toISOString() : null } as any).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["properties"] }),
  });
